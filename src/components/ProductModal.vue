...
let ModalInProductModal = null
let tempProductInProductModal = {}
const props = defineProps(['status', 'tempProduct'])
const emit = defineEmits(['emitProductModal', 'get-data'])
...
const confirmUpdate = () => {
  if (props.status === 'new') {
    axios
      .post(`${apiAdmin}/product`, { data: tempProductInProductModal })
      .then((res) => {
        alert(res.data.message)
        ModalInProductModal.hide()
        emit('get-data')
      })
      .catch((err) => alert(err.response.data.message))
  } else if (props.status === 'edit') {
    axios
      .put(`${apiAdmin}/product/${tempProductInProductModal.id}`, {
        data: tempProductInProductModal
      })
      .then((res) => {
        alert(res.data.message)
        ModalInProductModal.hide()
...
watch(props, (newProps, oldProps) => {
  tempProductInProductModal = newProps.tempProduct
})
onMounted(() => {
  productModal = new Modal(document.getElementById('productModal'), {
    keyboard: false
  })
  ModalInProductModal = productModal
  emit('emitProductModal', productModal)
  tempProductInProductModal = props.tempProduct
})
</script>
